document.ready; {
    ...
    var letter = userText.textContent.toLowerCase;
    var answerArray = [];
    var incorrect = 0;
    var rand = document.getElementById("chosenWord");
    document.onkeyup = function (event) {
        userText.textContent = event.key;
        //keyLog.push(userText.textContent);
        //console.log(keyLog);
    };
...
//takes two clicks to show at first, but it works
    ...
    function check() {
        for (var c = 0; c < rand.length; c++) {
            if (rand[c].toLowerCase === letter) {
                answerArray[rand[c]].innerHTML === letter;
    ...
    // function addGuesses() {
    // for (var y=0; y > keyLog.length; y++) {
    // createElement(keyLog[y]);
    // document.getElementById("gueses").appendChild;
    //     }
    // };
};